...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mode='Weighted sum'):
        self.world = world
        self.mode  = mode
        self.scale = scale
        ...
        self.panic_radius       = self.scale * 1
    ...
    def weighted_sum(self, target_position, delta):
        steering_force = Vector2D();
        local_group = self.get_local_group(self.local_group_radius)

        steering_force += self.separation_multiplier * self.seperation(local_group)
    ...
    def seperation(self, local_group):
